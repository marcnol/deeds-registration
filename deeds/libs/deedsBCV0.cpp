...
// Reproduce previous function, but modifies flow fields instead
...
void deeds_fields(float *im1, float *im1b, float *warped1, float *ux, float *vx, float *wx, int m, int n, int o, float alpha, int levels, bool verbose)
{
    ...
    // flow-fields are passed as arguments in this function
    // initial flow-fields are set to 0; i indicates backward (inverse) transform
    // u is in x-direction (2nd dimension), v in y-direction (1st dim) and w in z-direction (3rd dim)
    ...
    // here we don't wrap the moving image given the flow-field to save computation
...
// Reproduce previous functions, modifies image warped and flow fields
